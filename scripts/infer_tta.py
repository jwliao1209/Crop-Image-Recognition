...
def predict_tta(model, loader, savename, data_id):
    device = torch.device("cuda")
    pred = []
    tta_transforms = tta.Compose([tta.Rotate90(angles=[0, 90, 180, 270])])
    tta_model = tta.ClassificationTTAWrapper(model.to(device), tta_transforms)

    for batch_data in tqdm(loader):
        img  = batch_data["image"].to(device)
        pred.append(torch.argmax(tta_model(img), dim=1))
    ...
    infer_result = pd.DataFrame(columns=name, data = results)
    infer_result["label"] = infer_result["label"].map(mapping)
    infer_result.to_csv('../public/tta_'+savename+'.csv',index=None)
...
def main(args):
    ...
    predict_tta(model, loader, config["name"], ids)
